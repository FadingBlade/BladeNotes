#!/usr/bin/env sh
# BladeNotes installer — Linux / macOS
# Usage: curl -fsSL https://raw.githubusercontent.com/FadingBlade/BladeNotes/main/install.sh | sh
...
echo ""
printf "${BOLD}${CYN}BladeNotes installer${RST} — terminal notes manager\n"
echo ""
...
say "Downloading BladeNotes..."
if command -v curl >/dev/null 2>&1; then
    curl -fsSL "$REPO/BladeNotes.py" -o "$INSTALL_DIR/BladeNotes.py"
elif command -v wget >/dev/null 2>&1; then
    wget -q "$REPO/BladeNotes.py" -O "$INSTALL_DIR/BladeNotes.py"
else
    fail "curl or wget is required."
fi
chmod +x "$INSTALL_DIR/BladeNotes.py"
ok "Downloaded BladeNotes.py"

# ── Create notes wrapper ──────────────────────────────────────────────────────

cat > "$INSTALL_DIR/notes" << WRAPPER
#!/usr/bin/env sh
exec "$PYTHON" "$INSTALL_DIR/BladeNotes.py" "\$@"
WRAPPER
chmod +x "$INSTALL_DIR/notes"
ok "Created notes command"
...
echo ""
ok "${BOLD}BladeNotes installed!${RST}"
echo ""
printf "  ${CYN}notes new 'my first note'${RST}    create a note\n"
printf "  ${CYN}notes list${RST}                   see all notes\n"
printf "  ${CYN}notes read <id>${RST}              read a note\n"
printf "  ${CYN}notes update${RST}                 update BladeNotes\n"
printf "  ${CYN}notes help${RST}                   all commands\n"
echo ""
